...
export class FilterView extends React.Component<Props, State> {
    ...
    public updateFilter(categoryFilter: PhotoFilter, subFilter: string): void {
        this.props.store!.storyStore.filterStoriesToDisplay(categoryFilter, subFilter);
    }
        ...
        return (
            <div>
                <div
                    ...
                    onClick={() => this.setState({ currentFilter: this.state.currentFilter === cat ? '' : cat })}
                >
                    <p>{cat}</p>
                </div>
                {
    ...
    public render(): JSX.Element {
        return (
            <div style={{ width: 450, height: '100%', backgroundColor: '#00000011', paddingTop: 250 }}>
                {this.renderFilterButton('ZERO', ['ONE', 'TWO', 'THREE'])}
                {this.renderFilterButton('TEN', ['ELEVEN', 'TWELVE', 'THIRTEEN'])}
                {this.renderFilterButton('TWENTY', ['TWENTY-ONE', 'TWENTY-TWO', 'TWENTY-THREE'])}
            </div>
        );
    }
}
